#!/usr/bin/env python3
"""
SCF benchmark with Python thin wrapper for basis set loading.

Heavy compute (form_G, form_F, form_C, form_D, DirectJK, Libint2) runs in C++.
Use perf to profile: perf record -g -F 99 -- python run_scf_benchmark.py --threads 8 --single-iter
"""
...
# Default: benzene dimer cc-pVDZ (~300 NBF), suitable for 6226R-level (16-core) CPU
...
def parse_args():
    p = argparse.ArgumentParser(description="SCF benchmark (Python basis wrapper)")
    p.add_argument("--threads", type=int, default=1)
    p.add_argument("--repeat", type=int, default=1)
    p.add_argument("--single-iter", action="store_true",
                   help="Run SCF with maxiter=1 (one iteration) for perf focus")
    p.add_argument("--scf-type", default="direct", choices=["direct", "pk", "DIRECT", "PK"],
                   help="SCF_TYPE: direct or pk")
...
def run_one(args):
    ...
    if args.geometry:
        ...
        geom_path = os.path.join(os.path.dirname(__file__), "cases", "benzene_dimer.xyz")
...
def main():
    ...
    for run in range(1, args.repeat + 1):
        ...
        line = ("run={} threads={} nbf={} elapsed_s={:.8f} energy={:.16f}".format(
            run, args.threads, result["nbf"], result["elapsed_s"], result["energy"]))
        ...
        if args.csv_file:
            ...
            with open(args.csv_file, "a") as f:
                if write_header:
                    f.write("run,threads,nbf,elapsed_s,energy\n")
                f.write("{},{},{},{:.8f},{:.16f}\n".format(
                    run, args.threads, result["nbf"], result["elapsed_s"], result["energy"]))
    ...
    try:
        ...
        with open("timer.dat") as t:
            for ln in t:
                if any(x in ln for x in ("form_G", "form_F", "form_C", "form_D", "form_H", "DirectJK", "JK")):
